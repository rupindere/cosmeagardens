...
class Ysm_DB {
	...
	public static function filter_posts_where( $where ) {
		...
		if ( ! self::$is_postmeta_only ) {
			...
			if ( \Ysm_Search::get_var( 'taxonomies' ) ) {
				...
				if ( \Ysm_Search::get_post_types( 'product_variation' ) && 'parent' !== \Ysm_Search::get_var( 'product_variation_visibility' ) ) {
					// product variations
					$where_parts[] = sprintf( "( {$wpdb->posts}.post_type = 'product_variation' AND {$wpdb->posts}.post_parent IN (
							...
							LEFT JOIN {$wpdb->terms} t ON t.term_id = t_tax.term_id
							WHERE t_tax.taxonomy IN (%s) AND (%s)
						) )", implode( ',', $filtered_taxs ), self::make_like_query( 'lower(t.name)' ) );
				}
			}
		}
		...
		if ( class_exists( 'WooCommerce' ) ) {
			...
			if ( \Ysm_Search::get_post_types( 'product_variation' ) ) {
				...
				// exclude products that have variations and include only their children while simple products should be included
				if ( 'variations' === \Ysm_Search::get_var( 'product_variation_visibility' ) ) {
					$where_parts[] = "{$wpdb->posts}.ID NOT IN (
						SELECT DISTINCT post_parent
						FROM {$wpdb->posts}
						WHERE post_parent > 0 AND post_type = 'product_variation'
					)";
				}
			}
		}
	...
	public static function filter_posts_orderby( $orderby ) {
		...
		if ( \Ysm_Search::get_var( 'search_sku_first' ) && is_numeric( \Ysm_Search::get_var( 's' ) ) && self::$is_postmeta_only ) {
			$orderby = "CAST( {$wpdb->postmeta}.meta_value AS DECIMAL ) ASC, " . $orderby;
		}
	...
	protected static function make_like_query( $field ) {
		...
		if ( \Ysm_Search::get_var( 'skip_punctuation' ) ) {
			$patterns = [
				'!',
				'(',
				')',
			...
			foreach ( $patterns as $pp ) {
				$field = "REPLACE($field, '" . esc_sql( $pp ) . "', '')";
			}
		}

		foreach ( \Ysm_Search::get_search_terms() as $s_word ) {
			if ( \Ysm_Search::get_var( 'skip_punctuation' ) ) {
				$s_word = str_replace( '\'', '', $s_word );
				foreach ( $patterns as $pp ) {
					$s_word = str_replace( $pp, '', $s_word );
				}
			}
