...
class Forminator_Admin {
	...
	/**
	 * Displays an admin notice when Forminator version is 1.16.0 or higher and Stripe Addon version is less than 1.0.3
	 * Shown in forminator pages. Per user notification.
	 */
	public function check_stripe_addon_version() {
		// Show the notice only if Stripe Addon is active and its version is less than 1.0.3.
		if ( ! defined( 'FORMINATOR_STRIPE_ADDON' ) || ! class_exists( 'Forminator_Stripe_Addon' )
				|| version_compare( FORMINATOR_STRIPE_ADDON, '1.0.3', '>=' ) ) {
			return;
		}
		...
		$message .= sprintf( esc_html__( 'We\'ve noticed you have updated to Forminator Pro version %s. Please ensure you also update your Forminator Stripe Subscriptions Add-on to version 1.0.3 or higher to ensure compatibility with the new submissions processes.', 'forminator' ), FORMINATOR_VERSION );
		$message .= '</p>';
