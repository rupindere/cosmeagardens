...
class CR_Reminders_List_Table extends WP_List_Table {
	...
	public function no_items() {
		if( 'cr' === get_option( 'ivole_scheduler_type' ) ) {
			...
			echo sprintf( __( 'The plugin is configured to use CR Cron for scheduling review reminders (%1$s\'Reminders Scheduler\' setting%2$s).', 'customer-reviews-woocommerce' ), '<a href="' . admin_url( 'admin.php?page=ivole-reviews-settings' ) . '" title="Plugin Settings">', '</a>' );
			echo ' ';
			/* translators: please keep '%1$s' and '%2$s' as is   */
