            ...
            $( ".variations_form" ).on( "wc_variation_form woocommerce_update_variation_values", function() {
                ...
                $( "table.variations select" ).each( function() {
                    ...
                    $( "select[name=" + selName + "] option" ).each( function() {
                        ...
                        //var pricB = '${label}';
                        if (value=='Classic'){
                            var optionHtml = `<label class="generatedRadios${selClass}">${label}<img src="<?php echo $img_arr['Classic'];?>"><?php echo get_woocommerce_currency_symbol().$price_arr['Classic']; ?></label>`;
                        }
                        if (value=='Deluxe'){
                            var optionHtml = `<label class="generatedRadios${selClass}">${label}<img src="<?php echo $img_arr['Deluxe'];?>"><?php echo get_woocommerce_currency_symbol().$price_arr['Deluxe']; ?></label>`;
                        }
                        if (value=='Premium'){
                            var optionHtml = `<label class="generatedRadios${selClass}">${label}<img src="<?php echo $img_arr['Premium'];?>"><?php echo get_woocommerce_currency_symbol().$price_arr['Premium']; ?></label>`;
                        }
                        select.parent().append( $( optionHtml ).click( function() { 
